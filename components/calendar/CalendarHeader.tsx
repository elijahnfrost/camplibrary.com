...
// title (+ the "Color by" lens pill) on the left, then a right cluster of the
// view switch and the Add button. The [Day | Week | Month] switch is the same
// seg-slide the Library uses for Shelf/Deck/Catalog, so the two views' top bars
// match. There's no prev/next pager — navigation is the sidebar mini-month (and
// horizontal scroll), which also hosts the Today jump on desktop; the header
// keeps a Today button only on mobile, where the mini-month isn't rendered (CSS
// hides it from 768px up). View SETTINGS (weekends, week-start, days, camp
// hours) AND the Subscribe / .ics feed control live in the sidebar on desktop
// and behind the settings button → a sheet on mobile. Keyboard (wired in
