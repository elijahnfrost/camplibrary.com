...
(function () {
  ...
  function ageGroups(a) { return AGE_GROUPS.filter(function (g) { return (a.ages || []).indexOf(g.id) >= 0; }); }
  function ageSpan(a) {
    var gs = ageGroups(a); if (!gs.length) return "Grades 4–6";
    var grades = gs.filter(function (g) { return g.lo > 0; });
    var hasPre = gs.some(function (g) { return g.id === "pre"; });
    if (!grades.length) return "Preschool";
    var lo = Math.min.apply(null, grades.map(function (g) { return g.lo; }));
    var hi = Math.max.apply(null, grades.map(function (g) { return g.hi; }));
    if (hasPre) return "PreK–Gr " + hi;
    return "Grades " + lo + "–" + hi;
  }
  function ageLabel(a) { return ageSpan(a); }
  function ageStamps(a) { return ageGroups(a).map(function (g) { return g.short; }); }
  function codeAge(a) {
    var gs = ageGroups(a); var grades = gs.filter(function (g) { return g.lo > 0; });
    var hasPre = gs.some(function (g) { return g.id === "pre"; });
  ...
  ACTIVITIES.forEach(function (a) { a.rating = SEED_RATINGS[a.id] || 3; });

  // Derive age-group tags from the original min/max ranges.
  function deriveAges(a) {
    if (a.ages && a.ages.length) return;
    var ids = [];
    AGE_GROUPS.forEach(function (g) { if (a.ageMin <= g.max && a.ageMax >= g.min) ids.push(g.id); });
    a.ages = ids.length ? ids : ["g46"];
  }
  ACTIVITIES.forEach(deriveAges);
