...
const APP_AGES = window.CampData.AGE_GROUPS;
...
function App() {
  ...
  const [ratings, setRatings] = React.useState(() => store.get("camp:ratings", {}));
  ...
  React.useEffect(() => { store.set("camp:ratings", ratings); }, [ratings]);

  const all = React.useMemo(() => {
    const base = [...extra, ...ACTIVITIES];
    return base.map((a) => (ratings[a.id] != null ? Object.assign({}, a, { rating: ratings[a.id] }) : a));
  }, [extra, ratings]);
  ...
  const setRating = (id, val) => setRatings((p) => Object.assign({}, p, { [id]: val }));
  ...
  return (
    <div className="app">
      ...
      {tab === "library" && (
        <React.Fragment>
          ...
          <div className="filterbar">
            ...
            <span style={{ flex: "none", width: 1.5, background: "var(--line)", margin: "3px 4px" }} />
            {APP_AGES.map((g) => (
              <button key={g.id} className={"chip" + (age === g.id ? " is-on" : "")} onClick={() => setAge((cur) => (cur === g.id ? "All" : g.id))}>{g.short}</button>
            ))}
          </div>
        </React.Fragment>
      )}
      ...
      {/* overlays */}
      {detail && (
        <DetailSheet activity={byId[detail.id] || detail} isFav={isFav} onToggleFav={toggleFav} onClose={() => setDetail(null)}
          onAddToSchedule={addToSchedule} added={justAdded === detail.id} onSetRating={setRating} />
      )}
      {pickerSlot && (
