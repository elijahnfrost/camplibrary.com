...
const CATEGORIES = ["Game", "Craft", "Song", "Water", "Quiet", "Routine"];
...
// Hand-authored lane modules that are NOT regenerated from the JSON — they ship
// staples (reserved diagram-coupled games) and the classic games/crafts/routines
// lanes. They are always imported first in index.ts and their ids/titles seed the
// dedup sets so a future bulk regeneration never overwrites them or ships a
// duplicate. Keep this in sync with the hand-authored files under lib/seed/lanes.
const STATIC_LANES = [
  { file: "reserved", varName: "reservedActivities" },
...
// for both the pretty-printed reserved.ts and the minified one-line lanes). Used
// to seed seenIds/seenTitles so generated events can't collide with the staples.
function collectStatic() {
  const ids = new Set();
  const titles = new Set();
  let count = 0;
  for (const { file } of STATIC_LANES) {
    let src;
    try {
      src = readFileSync(join(LANES_DIR, file + ".ts"), "utf8");
    } catch {
      continue;
    }
    const pair = /["']?id["']?\s*:\s*["'][^"']+["']\s*,\s*["']?title["']?\s*:\s*["']([^"']+)["']/g;
    let m;
...
function main() {
  ...
  // Seed the dedup sets from every hand-authored static lane so a bulk regen can
  // never overwrite a staple or ship a duplicate of one (e.g. a generated "Tug of
  // War" is dropped because the classic-games lane already owns that title).
  const staticSeed = collectStatic();
  const seenIds = new Set([...RESERVED_IDS, ...staticSeed.ids]);
  const seenTitles = new Set(staticSeed.titles);
  const laneVars = [];
  let total = 0;
  const perLane = {};
  ...
  // index.ts — aggregate the hand-authored static lanes (first, so the staples
  // and classics sort ahead of the bulk catalog) + every generated lane.
  const imports = [`import type { Activity } from "@/lib/types";`];
  for (const { file, varName } of STATIC_LANES) imports.push(`import { ${varName} } from "./lanes/${file}";`);
  for (const { prefix, varName } of laneVars) imports.push(`import { ${varName} } from "./lanes/${prefix}";`);
  const spread = [...STATIC_LANES.map((l) => "..." + l.varName), ...laneVars.map((l) => "..." + l.varName)];
  ...
  console.log(JSON.stringify({ total: total + staticSeed.count, lanes: perLane, files: laneVars.length + STATIC_LANES.length + 1 }, null, 2));
}
